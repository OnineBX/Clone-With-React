...
type Props<T extends Route> = SceneRendererProps &
  RXEventEmitterProps & {
    ...
    sticky: boolean; // sticky means component will respond enter and leave events
  };
...
> {
  ...
  componentDidMount() {
    if (this.props.lazy || this.props.sticky) {
      // If lazy mode is enabled, listen to when we enter screens
      this.props.addListener('enter', this.handleEnter);
      this.props.addListener('leave', this.handleLeave);
  ...
  componentDidUpdate(prevProps: Props<T>, prevState: State) {
    ...
    if (
      ...
      if(!this.props.sticky) { // if not sticky listener, then remove or add
        if (this.props.lazy && this.state.loading) {
          ...
          this.props.removeListener('leave', this.handleLeave);
        }
      }
  ...
  // Ryan: add left event handler
  private handleLeave = (value: number) => {
      
    const { index } = this.props;
    // console.log('I am ' + index + ', leave ' + value + ",loading=" + this.state.loading)
    // If we're leaving the current route, we need to reset it
    if (value === index && !this.state.loading) {
        // reloading smoothly
