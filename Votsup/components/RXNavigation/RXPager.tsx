          ...
          ([isSwiping, indexAtSwipeEnd, currentIndex]: readonly number[]) => {
            ...
            if (isSwiping === TRUE) {
              ...
              if (keyboardDismissMode === 'auto') {
                ...
                /* Ryan add 'as React.ElementRef<HostComponent<unknown>>' 25/02/2021  */
                TextInput.State.blurTextInput(input as React.ElementRef<HostComponent<unknown>>);

                // Store the id of this input so we can refocus it if gesture was cancelled
                /* Ryan add 'as number' 25/02/2021 Ryan */
              ...
              if (keyboardDismissMode === 'auto') {
                if (indexAtSwipeEnd === currentIndex) {
                  ...
                  if (input) {
                      // Ryan add 'as unknown as React.ElementRef<HostComponent<unknown>>' 25/02/2021
                    TextInput.State.focusTextInput((input as unknown) as React.ElementRef<
...
const styles = StyleSheet.create({
  container: {
    // flex: 1, // comment by Ryan 26/02/2021
    position:'absolute',
    width:'100%',
    height: '100%',
    flexDirection: 'row',
