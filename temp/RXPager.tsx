import { NavigationHelpersContext, useNavigation, useNavigationState } from '@react-navigation/native';
...
import { GestureHandlerGestureEventNativeEvent, PanGestureHandler, State } from 'react-native-gesture-handler';
import Animated, {
  Easing as OldEasing,
  // @ts-ignore
  EasingNode,
...
export default class RXPager<T extends Route> extends React.Component<Props<T>, ComponentState> {
  static defaultProps = {
  ...
  private layoutHeight = new Value(this.props.layout.height);
  private tabBarHeight = 48;
          ...
          ([isSwiping, indexAtSwipeEnd, currentIndex]: readonly number[]) => {
            ...
            if (isSwiping === TRUE) {
              ...
              if (keyboardDismissMode === "auto") {
                ...
                /* use input as React.ElementRef<HostComponent<unknown>> here for type check 25/02/2021 Ryan */
                TextInput.State.blurTextInput(
                  input as React.ElementRef<HostComponent<unknown>>
                );

                // Store the id of this input so we can refocus it if gesture was cancelled
                /* use input as number here for type check 25/02/2021 Ryan */
              ...
              if (keyboardDismissMode === "auto") {
                if (indexAtSwipeEnd === currentIndex) {
                  ...
                  if (input) {
                    // use input as as unknown as React.ElementRef<HostComponent<unknown>> here for type check 25/02/2021 Ryan
                    TextInput.State.focusTextInput(
                      (input as unknown) as React.ElementRef<
